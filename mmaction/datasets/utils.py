...
def parse_hmdb51_splits(level):
    anno_dir = 'data/hmdb51/annotations'
    txt_list = []
    txt1_list = []
    txt2_list = []
    txt3_list = []
    for dirs, _, files in os.walk(anno_dir):
        for file in files:
            if file.endswith(".txt"):
                txt_list.append(file)
                filename = file.split(".")[0]
                if filename.endswith("1"):
                    txt1_list.append(file)
                elif filename.endswith("2"):
                    txt2_list.append(file)
                else:
                    txt3_list.append(file)
    #print(txt1_list, txt2_list, txt3_list)
    get_class_name = lambda x: "_".join(x.split("_")[:x.split("_").index("test")])
    names = sorted(list(set(list(map(get_class_name, txt_list)))))
    class_mapping = dict(zip(names, range(len(names))))
    print(class_mapping)

    def line_2_vidname(line):
        items = line.strip().split(' ')
        vid = items[0].split('.')[0]
        train_test = int(items[1])
        return vid, train_test
    splits = []
    for split in [txt1_list, txt2_list, txt3_list]:
        train_list = []
        test_list = []
        for file in split:
            class_name = "_".join(file.split("_")[:file.split("_").index("test")])
            label = class_mapping[class_name]
            #print(class_name)
            filepath = os.path.join(anno_dir, file)
            with open(filepath, "r") as f:
                lines = f.read()
                lines = lines.split("\n")
                lines = list(filter(None, lines))
                for line in lines:
                    vid, train_test = line_2_vidname(line)
                    vid = os.path.join(class_name, vid)
                    if train_test == 1:
                        train_list.append((vid, label))
                    elif train_test == 2:
                        test_list.append((vid, label))
                    else:
                        pass
        splits.append((train_list, test_list))
...
def parse_mmact_splits(level):
    class_ind = [x.strip().split()
                 for x in open('data/mmact/annotations/classInd.txt')]
    class_mapping = {x[1]: int(x[0]) - 1 for x in class_ind}
    def line2rec(line):
        items = line.strip().split(' ')
        vid = items[0].split('.')[0]
    ...
    for i in range(1, 4):
        train_list = [line2rec(x) for x in open(
            'data/mmact/annotations/train{:02d}.txt'.format(i))]
        test_list = [line2rec(x) for x in open(
            'data/mmact/annotations/test{:02d}.txt'.format(i))]
        splits.append((train_list, test_list))
    return splits
#splits = parse_mmact_splits(5)
#print(splits)


def parse_kinetics_splits(level):
    csv_reader = csv.reader(
        open('data/kinetics400/annotations/train.csv'))
    # skip the first line
    next(csv_reader)
    ...
    csv_reader = csv.reader(
        open('data/kinetics400/annotations/validate.csv'))
    next(csv_reader)
    val_list = [list2rec(x) for x in csv_reader]
    csv_reader = csv.reader(
        open('data/kinetics400/annotations/test.csv'))
    next(csv_reader)
